...
use sqlparser::{ast::{visit_statements, Expr, ExprWithAlias, GroupByExpr, Select, SelectItem, Statement, TableFactor}, dialect::Dialect};
use wasm_bindgen::prelude::wasm_bindgen;
use crate::binding::StmtAnalyzer;

static AGGREGATE_FUNC_NAMES : &[&str] = &["AVG", "COUNT", "MIN", "MAX", "SUM", "TOTAL"];
...
//External libraries exist, but create quite the mess code wise, so I rather not
pub trait Issue
{
...
impl<'a> StatementValues<'a>
{
    pub fn new() -> StatementValues<'a>
    {
...
impl StmtAnalyzer
{
    ...
    pub fn analyze_ast(&self) -> Result<Vec<AnalyzerReport>, String>
    {
        ...
        issues.append(&mut self.check_distinct_and_group_by());

        Ok(vec!())
    }

    fn check_distinct_and_group_by(&self) -> Vec<Box<dyn Issue>>
    {
